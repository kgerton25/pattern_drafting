...
#' 
#' @description 
#' This function returns the major pattern points of a pants front
...
pants_front <- function(crotch_length,
  ...
  ## LEG SLIMMING POINTS
  inseam_points = data.frame(point = c("O", "M"),
                             x = c(O[1], M[1]),
                             y = c(O[2], M[2]))
  ...
  outseam_points = data.frame(point = c("P", "N"),
                              x = c(P[1], N[1]),
                              y = c(P[2], N[2]))
  ...
  # Assemble Data
  point_data <- c(A, B, C, D, E, F, G, H, I, J, K, L, M, N, O, P, Q, Q1, E1, H1, p1, knee, p5, p6, d1, d2, d3, d4, d1m, d2m, W1)
  ...
  pattern <- points %>%
    ...
    geom_segment(aes(x = p6[1], y = p6[2], xend = P[1], yend = P[2])) +
